...
- **작업 도구 탭:** 선택 객체 정보를 맨 위에 표시하고, 실제 사용 기록에 따라 정렬되는 스마트 도구와 처음부터 쓸 수 있는 기본 도구를 이어서 배치합니다.
...
- AutoCAD에 명령이 정상적으로 전달된 경우에만 횟수를 올립니다.
...
초기 버전은 `dotnet clean`/`dotnet build`로 검증했습니다. UI 정리 버전은 기존 DLL이 실행 중인 AutoCAD에 잠겨 있어 Release 빌드로 검증했습니다. AutoCAD를 로드하지 않는 별도 WPF 미리보기에서 320px/370px 폭, 두 메인 탭과 스마트 도구 탭, 그룹 펼치기/접기, 선택 없음, 데이터 바인딩과 36개 명령의 중복 없는 구성을 확인했습니다. 사용 기록의 저장·재로딩·2회 기준 집계·초기화도 별도 테스트했습니다. 미리보기 수치는 화면 검증용 예시이며 실제 도면 분석 결과가 아닙니다.
